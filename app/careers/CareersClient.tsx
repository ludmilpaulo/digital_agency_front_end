...
// Lottie does DOM work; load it only on the client.
...
export default function CareersClient() {
  ...
  // Client-only language detection
  useEffect(() => {
    const lang = navigator.language?.toLowerCase?.() ?? "en";
    const detected = (lang.startsWith("pt") ? "pt" : "en") as "en" | "pt";
    setLanguage(detected);
    setForm((prev) => ({ ...prev, language: detected }));
  }, []);

  // Fetch careers on the client (avoid SSR fetch to prevent prerender issues)
  useEffect(() => {
    let active = true;
    ...
    async function load() {
      try {
        ...
        const res = await fetch(`${baseAPI}/careers/careers/`, { signal: ctrl.signal });
        if (!res.ok) throw new Error(`Failed to load jobs (${res.status})`);
        const data = (await res.json()) as Career[];
        if (active) setJobs(Array.isArray(data) ? data : []);
      } catch (e: any) {
        if (e?.name !== "AbortError") {
          // Fallback: keep jobs empty; UI will show "No jobs match"
                ...
                >
                  <Dialog.Panel className="w-full max-w-lg xs:max-w-xl sm:max-w-3xl transform overflow-hidden rounded-2xl sm:rounded-3xl bg-white p-4 xs:p-6 sm:p-8 shadow-2xl transition-all animate-fade-in-up border border-blue-100">
                    ...
                    <div className="prose max-w-none mb-6">
                      <h3 className="text-base xs:text-lg font-bold mt-4 mb-2 text-blue-800">
                        Description
                      </h3>
                      <div
                      ...
                      />
                      <h3 className="text-base xs:text-lg font-bold mt-6 mb-2 text-blue-800">
                        Requirements
                      </h3>
                      <div
                        dangerouslySetInnerHTML={{
                    ...
                    <form onSubmit={handleSubmit} className="space-y-3 xs:space-y-4 mt-3 xs:mt-6">
                      <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 xs:gap-4">
                        <div>
                          <label className="block text-xs xs:text-sm font-medium mb-1">
                          ...
                          <select
                            ...
                            value={form.language}
                            onChange={(e) => {
                              const lang = (e.target.value as "en" | "pt") || "en";
