...
export default function MyTasks() {
  ...
  // By default, show logged-in user's tasks, but can view all
  ...
  // DnD reorder logic (simulate order, could call API for real order update)
  function reorder<T>(arr: T[], start: number, end: number): T[] {
    const copy = [...arr];
  ...
  // Keep DnD state in sync with data
  useEffect(() => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const sections = ["today", "week", "others"] as const;
    ...
    return (
      <Draggable draggableId={String(task.id)} index={index}>
        {(provided) => (
          ...
          >
            <div className="flex items-center justify-between px-4 py-2 cursor-pointer" onClick={onToggle}>
              <div className="flex flex-col gap-1">
                <div className="font-bold text-blue-900 flex items-center gap-2 text-base">
                  <span>{task.title}</span>
                  <span className={clsx("rounded px-2 py-0.5 text-xs font-medium ml-1", statusColors[task.status])}>
                    {task.status}
                  </span>
                  {task.priority && (
                    <span className={clsx("rounded px-2 py-0.5 text-xs font-medium", priorityColors[task.priority || "Medium"])}>
                      <Flag size={14} className="inline-block mr-1" />{task.priority}
                    </span>
                  )}
                </div>
                {task.due_date && (
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="ml-2 text-blue-600 hover:text-blue-900 transition" aria-label="Expand">
                ...
                <div className="flex items-center gap-2 mb-2">
                  {task.assignees?.length > 0 ? (
                    task.assignees.map((u: User) => (
                      <span key={u.id} className="flex items-center gap-1 bg-blue-50 px-2 rounded-full text-xs font-semibold text-blue-700 border">
  ...
  return (
    <div className="max-w-3xl mx-auto px-2 py-8 relative">
      ...
      <AddTaskModal
        ...
        onCreated={() => {/* Optionally refresh tasks or show toast */}}
      />
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <button
            className={clsx(
              ...
              showOnlyMine ? "bg-blue-600 text-white border-blue-600" : "bg-white text-blue-700 border-blue-400"
            )}
          ...
          {(provided) => (
            <section className="mb-8" ref={provided.innerRef} {...provided.droppableProps}>
              <div className="flex items-center gap-2 mb-2 font-bold text-blue-700 text-lg">
                <span>Today</span>
                <span className="bg-blue-100 text-blue-600 px-2 rounded text-xs">{dragged.today.length}</span>
              </div>
              <div className="space-y-3">
                {dragged.today.length === 0 && <div className="text-xs text-gray-400 pl-2">No tasks for today.</div>}
        ...
        <Droppable droppableId="week">
          {(provided) => (
            <section className="mb-8" ref={provided.innerRef} {...provided.droppableProps}>
              <div className="flex items-center gap-2 mb-2 font-bold text-blue-700 text-lg">
                <span>This Week</span>
                <span className="bg-blue-100 text-blue-600 px-2 rounded text-xs">{dragged.week.length}</span>
              </div>
              <div className="space-y-3">
                {dragged.week.length === 0 && <div className="text-xs text-gray-400 pl-2">No tasks this week.</div>}
          ...
          {(provided) => (
            <section ref={provided.innerRef} {...provided.droppableProps}>
              <div className="flex items-center gap-2 mb-2 font-bold text-blue-700 text-lg">
                ...
                <span className="bg-blue-100 text-blue-600 px-2 rounded text-xs">{dragged.others.length}</span>
              </div>
              <div className="space-y-3">
                {dragged.others.length === 0 && <div className="text-xs text-gray-400 pl-2">Nothing else pending!</div>}
