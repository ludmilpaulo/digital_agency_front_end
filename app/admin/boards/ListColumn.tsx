...
const ListColumn: React.FC<ListColumnProps> = ({
  list, boardUsers, newCardDetails, setNewCardDetails, handleAddCard, handleDeleteCard, handleDeleteList
}) => (
  <Draggable draggableId={String(list.id)} index={list.id}>
    {(listProvided) => (
      <div
      ...
      >
          ...
          {(dropProvided, snap) => (
            ...
            >
              {list.cards.map((card, j) => (
                <CardItem key={card.id} card={card} handleDeleteCard={handleDeleteCard} />
              ))}
        ...
        <CardAddForm
          ...
          details={newCardDetails[list.id] || { title: "", description: "", status: "Not Started", assignees: [] }}
          setDetails={d => setNewCardDetails(nc => ({ ...nc, [list.id]: d }))}
